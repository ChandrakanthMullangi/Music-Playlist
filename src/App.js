...
class App extends Component {
  ...
  onClickDeleteIcon = id => {
    ...
    const updatedTrackList = tracksList.filter(eachTrack => eachTrack.id !== id)
    this.setState({tracksList: updatedTrackList})
  }

  render() {
    const {searchInput, tracksList} = this.state

    const filteredTracksList = tracksList.filter(eachTrack =>
      eachTrack.name.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="music-playlist-app">
        <div className="music-playlist-image-container">
          <h1 className="name"> Ed Sheeran </h1>
          <p className="profession"> Singer </p>
        </div>
        <div className="songs-playlist-container">
          <div className="songs-playlist-header-section">
